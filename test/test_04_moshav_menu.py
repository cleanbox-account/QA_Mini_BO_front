...
# version ='1.0'
# ---------------------------------------------------------------------------
""" testing with pytest tool. Testing of Mni BO for Moshav users"""
# ---------------------------------------------------------------------------
import pytest
...
class TestMoshavimSuppliersBO(BaseClass):
    ...
    @pytest.mark.parametrize('role', [pytest.param('YadMordechai' ),   #yadmordehai
                                      pytest.param('Amirim' ),   #amirim
                                                pytest.param('SdeMoshe' )  #sde moshe
                                                    ])
    def test_04_for_moshav_menu_overview(self, role):
        ...
        if not logined_yes_not:
            assert  False
        elif role in ('YadMordechai','SdeMoshe','Amirim'):
