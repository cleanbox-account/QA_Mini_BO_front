...
# version ='1.0'
# ---------------------------------------------------------------------------
""" testing with pytest tool. Testing of Mini BO for Yad Mordehai - new order pages"""
# ---------------------------------------------------------------------------
import random
import pytest
...
class TestYadMordehaiBONewOrder(BaseClass):
    ...
    @pytest.mark.parametrize('role', [ pytest.param('YadMordechai' ),   #decathlon
                                                    ])
    def test_08_for_yad_mordehai_new_order(self, role):
        ...
        sleep(1)
        my_station=c_order_page.findStation("5900")
        if my_station:
        ...
        while not has_new_order:
            ...
            new_pcg_number="YadMrd"+str(random.randint(1000000, 9999999))
